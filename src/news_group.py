...
def read(dataset):
    ...
    if dataset["dataset"] == "all":
      training_data_tuples = get_data_tuples(os.path.join(dataset["path"], 'train.data'))
      training_labels = get_labels(os.path.join(dataset["path"], 'train.label'))
      testing_data_tuples = get_data_tuples(os.path.join(dataset["path"], 'test.data'))
      ...
      raise ValueError("dataset must be 'all' or 'testing' or 'training', not " + dataset["dataset"])
    labels *= 1.0
    num_data = sum(1 for k in range(DOC_NUM) if labels[k] in dataset["labels"])
    
    wc_sparse = csr_matrix(([data_tuples[i][2]*1.0
