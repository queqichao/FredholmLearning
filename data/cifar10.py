...
def read_images(dataset):
  ...
  if dataset["dataset"] == "training":
    ...
    for i in range(5):
      fname_data = os.path.join(dataset["path"], 'data_batch_'+str(i+1)+'.bin')
      with open(fname_data, 'rb') as fdata:
        data[i*10000:(i+1)*10000] = np.fromfile(fdata, dtype=np.uint8).reshape(10000, 3073)
  ...
  lbl = data[:,0]
  img = np.transpose(data[:,1:].reshape((data.shape[0], 3, 32, 32)), (0, 2, 3, 1))
  if "seed" in dataset:
