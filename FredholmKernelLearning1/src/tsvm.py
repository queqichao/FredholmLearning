...
class SVMLight(BaseEstimator, ClassifierMixin):
  def __init__(self, C=None, costratio=1.0, kernel='rbf', poly_degree=3, rbf_gamma=0.0, coef_lin=0.0, coef_const=0.0):
    self.C = C
  ...
  def fit(self, X, y):
    ...
    Y_unlabeled = np.zeros((num_unlabeled, self.num_classes_,), dtype=np.float32)
    ...
    for i in range(self.num_classes_):
      y_column = Y[:,i]
      self.model_.append(svm.learn(self.__data2docs(X, y_column), type='classification'.encode()))
  ...
  def predict(self, X):
    ...
    for i in range(self.num_classes_):
      scores[:,i] = svm.classify(self.model_[i], self.__data2docs(X, np.zeros((num_data,), dtype=np.float32)))
