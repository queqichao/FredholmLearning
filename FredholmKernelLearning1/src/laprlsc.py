...
class LapRLSC(BaseL2KernelClassifier):
  def __init__(self, rbf_gamma=None, normalize_laplacian=False, nu=1.0, nu2=None):
  ...
  def get_label_matrix(self, y):
    ...
    Y_unlabeled = np.zeros((num_unlabeled, Y_labeled.shape[1],), dtype=np.float32)
  ...
  def fit(self, X, y):
    ...
    if self.nu2 != 0:
      laplacian_x_kernel = np.dot(graph_laplacian(kernel_matrix_, normed=self.normalize_laplacian), kernel_matrix_)
      A += self.nu2*laplacian_x_kernel
    super(LapRLSC, self).fit(A, y)

  @profile
  def predict(self, X):
    kernel_matrix_ = rbf_kernel(util.cast_to_float32(X), self.X_, gamma=self.rbf_gamma_)
